...
const inviteTeamMemberByEmail = asyncHandler(async (req, res) => {
  ...
  if (!invitedUser) {
    res.status(404);
    throw new Error(
      "No IntellMeet account found for this email yet — they'll need to sign up first before you can add them."
    );
  }
...
const getMyInvites = asyncHandler(async (req, res) => {
  const invites = await Invite.find({ invitedClerkId: req.user.clerkId, status: 'pending' }).sort({
    createdAt: -1,
  });
