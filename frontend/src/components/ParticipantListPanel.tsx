...
export default function ParticipantListPanel({ onClose, hostClerkId, waitingRoom = [], isHost = false, onAdmit, onDeny, onAdmitAll }: ParticipantListPanelProps) {
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isHost && waitingRoom.length > 0 && (
          <div className="mb-3">
            ...
            >
              <span className="text-[13px] font-semibold uppercase tracking-wider">Waiting to be admitted</span>
            ...
            {isWaitingSectionOpen && (
              <div className="mt-1">
                ...
                {waitingRoom.map((w: any) => (
                  <div key={w.clerkId} className="flex items-center justify-between p-2 hover:bg-[#1E293B] rounded-lg transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      {w.profileImage ? (
                        <img src={w.profileImage} alt={w.name} className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
                      ) : (
                        <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0" style={{ background: getAvatarColor(w.clerkId) }}>
                          {getInitials(w.name)}
                        </div>
                      )}
        ...
        <div>
          <button
            ...
            className="w-full flex items-center justify-between px-2 py-2 text-[#94A3B8] hover:bg-[#1E293B] rounded-lg transition-colors"
          >
            <span className="text-[13px] font-semibold uppercase tracking-wider">In the meeting</span>
            <div className="flex items-center gap-2">
          ...
          {isMembersSectionOpen && orderedList.map((p) => {
